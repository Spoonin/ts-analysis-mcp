...
describe("find_hooks", () => {
  it("finds Redux hooks in ConnectedUserList", () => {
    const r = findHooks({ component: "ConnectedUserList" }, ctx());
  ...
  it("finds useState and useEffect in custom hook", () => {
    const r = findHooks({ component: "useAuth" }, ctx());
  ...
  it("returns empty hooks for component without hooks", () => {
    const r = findHooks({ component: "Button" }, ctx());
    expect(r.hooks).toEqual([]);
  });

  it("throws for unknown component", () => {
    expect(() => findHooks({ component: "NonExistent" }, ctx())).toThrow(
      /not found/i,
    );
  });
